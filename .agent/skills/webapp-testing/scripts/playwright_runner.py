...
def run_basic_test(url: str, take_screenshot: bool = False) -> Dict[str, Any]:
    ...
    try:
        with sync_playwright() as p:
            ...
            # Health checks
            is_loaded = response.ok if response else False
            result["health"] = {
                "loaded": is_loaded,
            ...
            result["status"] = "success" if is_loaded else "failed"
            if result["status"] == "success":
                result["summary"] = "[OK] Page loaded successfully"
            else:
                result["summary"] = "[X] Page failed to load"

    except Exception as e:
        error_msg = str(e)
        result["status"] = "error"
        result["error"] = error_msg
        result["summary"] = "[X] Error: {}".format(error_msg[:100])
...
def run_accessibility_check(url: str) -> Dict[str, Any]:
    ...
    result: Dict[str, Any] = {"url": url, "accessibility": {}}
